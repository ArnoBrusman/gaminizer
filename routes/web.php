...
//    Route::any('races/{any}', 'RacesController@allData');
    ...
    $standardApiRoutes = ['narratives', 'classes', 'races', 'proficiencies'];
    
    foreach ($standardApiRoutes as $route) {
        Route::group(['prefix'=> $route], function() use($route){
            Route::get('/', ucfirst($route) . 'Controller@index');
            Route::get('{id}', ucfirst($route) .'Controller@show');
            Route::put('{id}', ucfirst($route) .'Controller@update')->name('::update');
            Route::delete('{id}', ucfirst($route) .'Controller@delete')->name('::delete');
            Route::post('/', ucfirst($route) .'Controller@store')->name('::store');
        });
    }

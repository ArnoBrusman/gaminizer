...
use Gaminizer\Collections\PcCollection;
...
class Pc extends Model
{
    ...
    function stats()
    {
        return $this->hasMany('Gaminizer\PcStats');
    }
    
    function pcClasses()
    {
        return $this->hasMany('Gaminizer\PcClasses');
    }
    
    function classes()
    {
        return $this->belongsToMany('Gaminizer\CharacterClass', 'pc_classes', 'pc_id', 'class_id');
    }
    
    function pcFeatures()
    {
        return $this->hasMany('Gaminizer\PcFeatures');
    }
    
    function features()
    {
        return $this->belongsToMany('Gaminizer\Feature', 'pc_features');
    }
    
    function narrative()
    {
        return $this->hasOne('Gaminizer\Narrative');
    }
    
    function possessions()
    {
        return $this->belongsToMany('Gaminizer\Possession');
    }
    
    function proficiencies()
    {
        return $this->hasMany('Gaminizer\Proficiency');
    }
    ...
    function get_ability_scores($elaborate = false)
    {
        $known_atr = config('character.ability_scores');
        
        $stats = $this->stats();
        $ability_stats = $this->get_stats('AS_.*', $elaborate);

        $ability_scores = array();
        foreach ($known_atr as $value) {
            if(isset($ability_stats['AS_'.$value])) {
                $ability_scores['AS_'.$value] = $ability_stats['AS_'.$value];
            } else {
                $ability_scores['AS_'.$value] = 0;
            }
        }

...
class Character extends Model
{
  ...
  function get_classes($result) 
  {
    return $this->get_many_on_many($result, 'pc_classes', 'class', array('level', 'origin'));
  }
  
  function get_ability_scores($result, $elaborate = false)
  {
    $known_atr = config_item('ability_scores');
    $ability_stats = $this->get_stats($result, 'AS_.*', $elaborate);
   ...
   * @return type
   */
  function get_stats($result, $type = NULL, $elaborate = false ) 
  {
    $pc_stats = $this->get_related($result, 'pc_stats');
    $stats = array();
    
    if(is_null($type)) {
      $stat_types = $this->pc_stats_model->get_stat_types();
    } else {
      $stat_types = $this->pc_stats_model->get_stat_types('/^'.$type.'$/');
    }
    ...
    foreach ($stat_types as $stat_type) {
      ...
      if($elaborate) {
        ...
        $stats[$stat_type] = $this->pc_stats_model->calc_stat_type_value($pc_stats, $stat_type);
      }
    }
